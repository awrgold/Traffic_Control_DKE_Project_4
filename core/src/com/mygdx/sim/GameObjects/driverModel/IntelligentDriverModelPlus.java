...
public class IntelligentDriverModelPlus implements DriverModel {
	...
	public double determineAcceleration(TrafficManager mgr, Vehicle vehicle, int timestep) {
		...
		for (int i = 0; i < Math.min(lanesToSynchronizeWith.size(), 10); i++) {
			Location simulatedLocation = new Location(lanesToSynchronizeWith.get(i),
					vehicle.getDistanceOnEdge(timestep));
		...
		double desiredGap = LINEAR_JAM_DISTANCE + vehicleLength + safetyHeadway
		...
		for (int i = 0; i < Math.min(lanesToSynchronizeWith.size(), 10); i++)
			if (accelerationsDueToSynch[i] < acceleration)
				if (accelerationsDueToSynch[i] > safeDeceleration)
					...
					acceleration = safeDeceleration;

		return acceleration;
	}

	private double determineAcceleration(TrafficManager mgr, Vehicle vehicle, Location simulatedLocation,
			int timestep) {
		if (timestep == 0)
			return 0;
		...
		DistanceAndSpeed nextTrafficObject = mgr.getDistanceAndSpeedToClosestTrafficObject(vehicle, simulatedLocation,
				timestep);
		double distanceToNextTrafficObject = nextTrafficObject.getDistance();
		double speedOfNextTrafficObject = nextTrafficObject.getSpeed();
	...
	private double determineAcceleration(TrafficManager mgr, Vehicle vehicle, DistanceAndSpeed simulatedTrafficObject,
			int timestep) {
		if (timestep == 0)
			return 0;
